...
- **Asaas:** `ASAAS_ACCOUNTS_ENABLED`, `ASAAS_MASTER_API_KEY`, `PAYMENTS_ENCRYPTION_KEY`, `ASAAS_WEBHOOK_PUBLIC_BASE_URL` (seção em `.env.prod.example`; injetadas no compose da API). Sandbox na VPS: `ASAAS_ENV=sandbox`, `ASAAS_ALLOW_SANDBOX_IN_PROD=true` (remover no go-live).

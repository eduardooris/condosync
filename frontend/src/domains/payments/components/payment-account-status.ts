...
export type PaymentAccountStatusTone =
  | 'neutral'
  | 'info'
  | 'warning'
  | 'success'
 ...
 */
export const PAYMENT_ACCOUNT_STATUS_MAP: Record<
  PaymentAccountStatus,
  { label: string; tone: PaymentAccountStatusTone; icon: LucideIcon }
> = {
...
export function paymentAccountStatusIcon(status: PaymentAccountStatus): {
  ...
  const tone = PAYMENT_ACCOUNT_STATUS_MAP[status]?.tone ?? 'neutral';
  const className =
    tone === 'success'
      ? 'text-emerald-500'
      : tone === 'warning'
        ...
        : tone === 'danger'
          ...
          : tone === 'info'
            ? 'text-brand-500'
            : 'text-ds-subtle';
  return { Icon: PAYMENT_ACCOUNT_STATUS_MAP[status]?.icon ?? AlertTriangle, className };
}

export function paymentAccountStatusLabel(status: PaymentAccountStatus): string {
  return PAYMENT_ACCOUNT_STATUS_MAP[status]?.label ?? status;
}
